...
def parse_function(attr_text: str, name: str) -> FunctionInfo:
    _, kvs = parse_attributes(attr_text)
    return FunctionInfo(name=name, lua_name=kvs.get("LuaName"))
...
def emit_gen_cpp(source_header_include: str, classes: list[ClassInfo]) -> str:
    ...
    for c in classes:
        ...
        if c.functions:
            out.append(emit_lua_registrar(c))
    return "\n".join(out)
...
def emit_lua_registrar(c: ClassInfo) -> str:
    ...
    for i, fn in enumerate(c.functions):
        ...
        comma = "," if i < len(c.functions) - 1 else ""
